...
def test_case_timeout_remains_a_runtime_failure(tmp_path, monkeypatch):
    from aqours_code.command_executor import CaseTimeoutError

    executor = LocalCommandExecutor()

    def raise_timeout(_command, _cwd, _timeout):
        executor.command_execution_count += 1
        raise CaseTimeoutError("runtime deadline")

    monkeypatch.setattr(executor, "execute", raise_timeout)
    client = FakeClient([
