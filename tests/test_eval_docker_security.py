...
def test_docker_eval_policy_exposes_full_harness(monkeypatch):
    ...
    assert "search_archived_tool_results" not in names
    assert "read_archived_tool_result" not in names
...
def test_eval_trace_storage_is_separate_and_exposes_normal_process_metrics(
    ...
    assert set(metrics) == {
        ...
        "verifier_allocated_model_calls", "verifier_budget_mode",
        "verifier_global_calls_remaining_at_start",
        "verifier_local_model_call_limit", "verifier_local_tool_call_limit",
