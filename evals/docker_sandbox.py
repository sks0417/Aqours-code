...
class DockerAgentRunner:
    ...
    def docker_run_args(
        ...
        *,
        ...
        mounts = [
            bind_mount(agent_workspace, "/workspace"),
            bind_mount(agent_state, "/state"),
            bind_mount(runtime_root, "/runtime"),
    ...
    def run(self, **paths) -> tuple[subprocess.CompletedProcess, dict]:
        ...
        try:
            ...
            error = SandboxError(
                f"Docker Agent failed to start: {type(exc).__name__}: {exc}")
            raise error from exc
        finally:
            self._cleanup()
        if proc.returncode == 125:
            detail = (proc.stderr or proc.stdout or "docker run failed").strip()
            raise SandboxError(f"Docker Agent failed to start: {detail}")
        return proc, {
            ...
            "container_cleanup_succeeded": self.cleanup_succeeded,
