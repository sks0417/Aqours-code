from collections import defaultdict

from .errors import DuplicateExternalId
from .models import ImportResult
from .serialization import rows_fingerprint, serialize_result
from .validation import validate_rows


class InventoryImportService:
    def __init__(self, repository, dedupe):
        self.repository = repository
        self.dedupe = dedupe
        self._next_batch = 1

    def import_batch(self, payload, *, idempotency_key):
        rows = validate_rows(payload)
        fingerprint = rows_fingerprint(rows)
        cached = self.dedupe.lookup(idempotency_key, fingerprint)
        if cached is not None:
            return cached
        indexes = defaultdict(list)
        for index, row in enumerate(rows):
            indexes[row.external_id].append(index)
        duplicate = next((external_id for external_id, positions in indexes.items()
                          if len(positions) > 1), None)
        if duplicate is not None:
            raise DuplicateExternalId(duplicate, tuple(indexes[duplicate]))
        self.repository.insert_many(rows)
        result = serialize_result(ImportResult(
            f"batch-{self._next_batch:04d}", len(rows),
            tuple(row.external_id for row in rows),
        ))
        self._next_batch += 1
        self.dedupe.remember(idempotency_key, fingerprint, result)
        return result
