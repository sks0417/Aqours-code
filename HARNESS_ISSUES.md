...
| HARN-003 | P2 | Proposed | Docker lifecycle metrics | grader 未启动时，清理汇总显示失败，容易被误解为容器泄漏 | 2026-07-16 inventory stress Eval |
| HARN-004 | P2 | Open | Trace analyzers | 分析脚本使用旧 Trace schema，错误地把工具结果显示为长度 0 | 2026-07-16 inventory stress Eval |
| HARN-005 | P1 | Proposed | Docker Eval image | Eval 默认静默复用旧镜像，使运行结果与当前 Harness 源码不一致 | 2026-07-18 inventory stress Eval |
...
| HARN-007 | P2 | Proposed | LLM Trace accounting | Compact summary 模型调用未写入 LLM Trace，导致调用数和耗时不完整 | 2026-07-18 inventory stress Eval |
| HARN-008 | P2 | Fixed | Docker image build | Harness 源码变更会使依赖安装层失效，瞬时 PyPI TLS 故障阻断镜像刷新 | 2026-07-18 inventory stress Eval |
| HARN-009 | P2 | Fixed | Background task routing | bash 慢操作检测按任意子字符串匹配，使 `cat tests/...` 被错误后台化 | 2026-07-18 inventory stress Eval |
| HARN-010 | P2 | Proposed | Docker execution metrics | Agent 失败时容器结果不返回内部 command metadata，summary 错报 bash 执行数为 0 | 2026-07-18 inventory stress Eval |
...
代码与 scripted 集成测试已通过，因此状态为 `Fixed`。尚未重新构建镜像运行原始真实模型压力 case；
需要确认角色调用数、总 token、Reviewer 命中率、最终 grader 分数和 Worktree 清理后才能标记
`Validated`。

## 建议实施顺序

1. **HARN-015（Fixed）**：重新构建镜像，验证角色调用、预算消耗、Reviewer 结果和 Worker 清理闭环。
2. **HARN-012（Fixed）**：用 Reviewer 验证 producer-function/field-enumeration 审计能补上 quantity fingerprint。
3. **HARN-001（In progress）**：继续观察 full compact 后局部复读，并比较 Explorer 是否降低 Lead 重读。
4. **HARN-013（Validated）+ HARN-014（Validated）**：保持 final audit 范围和 recursive glob 的真实压力回归覆盖。
5. **HARN-002（Validated）**：保持 Agent 失败后 diagnostic grader 的真实压力回归覆盖。
6. **HARN-005**：增加 Runtime fingerprint，自动证明压力结果使用当前源码。
7. **HARN-009（Fixed）**：在原压力 case 验证普通 grep/调试命令保持同步。
8. **HARN-004**：修复 Trace analyzer schema，提高后续压力运行的诊断效率。
9. **HARN-007**：补齐全部模型调用的 Trace，确保预算和延迟分析可信。
10. **HARN-011（Validated）**：保持 pytest `task_notification` 的回归覆盖。
11. **HARN-010**：让异常失败路径也能返回真实命令执行指标。
12. **HARN-008（Fixed）**：用后续 Harness 源码修改验证依赖安装层持续命中缓存。
13. **HARN-003**：整理生命周期指标语义，避免错误运维判断。
