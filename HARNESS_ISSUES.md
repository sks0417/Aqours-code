...
> 2026-08 v0.1 简化：Acceptance gate、验收类 Todo、证据绑定和 Reviewer 自动改写
> Todo 已从运行时移除。未进入 Context 或决策链、却会在工具执行前后扫描全仓的
> RunKnowledge 也已移除；读取摘要指标改为直接写入 Trace。下文相关条目保留为历史
> Eval 记录，不再描述当前主循环设计。
